...
#define portSET_CRITICAL_NESTING_COUNT(x) \
  (xCriticalNestings[portGET_CORE_ID()] = (x))
...
#error \
    "configUSE_PORT_OPTIMISED_TASK_SELECTION requires configMAX_PRIORITIES <= 32"
#endif
...
#define portRELEASE_ISR_LOCK() \
  vPortRecursiveLock(portGET_CORE_ID(), 0u, pdFALSE)
#define portGET_TASK_LOCK() vPortRecursiveLock(portGET_CORE_ID(), 1u, pdTRUE)
#define portRELEASE_TASK_LOCK() \
  vPortRecursiveLock(portGET_CORE_ID(), 1u, pdFALSE)
#endif
