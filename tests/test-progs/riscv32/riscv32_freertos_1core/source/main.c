...
qemu-system-riscv32 -nographic -machine virt -net none -chardev stdio,id=con,mux=on -serial chardev:con -mon chardev=con,mode=readline -bios none -smp 4 -kernel build/demo.elf
...
int main( void )
{
	...
	xTaskCreate(test1_task, (portCHAR *)"test1_task", 512, NULL, 30, NULL);
